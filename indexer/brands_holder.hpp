...
template <class FnT> void ForEachLocalizedBrands(std::string_view brand, FnT && fn)
{
  ...
  /// @todo Remove temporary string with the new cpp standart.
  /// Localazied brands are not working as expected now because we store raw names from OSM, not brand IDs.
