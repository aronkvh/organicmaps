...
void PreprocessElement(OsmElement * p, CalculateOriginFnT const & calcOrg)
{
  ...
  static std::array<storage::CountryId, 3> const provinceToState = {
    "Japan", "South Korea", "Turkey"
  };
  ...
  p->UpdateTag("place", [&](string & value)
  {
    ...
    auto const countryId = infoGetter->GetRegionCountryId(org);
    if (!countryId.empty())
    {
      auto const country = storage::GetTopmostParentFor(*countryTree, countryId);
      if (std::find(provinceToState.begin(), provinceToState.end(), country) != provinceToState.end())
        value = "state";
    }
    else
      LOG(LWARNING, ("CountryId not found for (lat, lon):", mercator::ToLatLon(org), *p));
  });
...
void PostprocessElement(OsmElement * p, FeatureBuilderParams & params)
{
  ...
  // Foot attr is stonger than Sidewalk,
  // Bicycle attr is stonger than Cycleway,
...
// And most of this juctions are assumed to be oneway.
                    ...
                    TypesFilterFnT const & filterType, CalculateOriginFnT const & calcOrg)
{
...

  {
    ...
    if (params.FinishAddingTypesEx() == FeatureParams::TYPES_EXCEED_MAX)
      LOG(LWARNING, ("Exceeded types count for:", DebugPrintID(*p), "Types:", typesString));
  }
